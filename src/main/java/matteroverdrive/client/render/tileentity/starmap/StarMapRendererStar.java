...
public class StarMapRendererStar extends StarMapRendererAbstract {
    ...
    private void drawPlanetInfo(Planet planet) {
        ...
        if (GalaxyClient.getInstance().canSeePlanetInfo(planet, Minecraft.getMinecraft().player)) {
            ...
            Minecraft.getMinecraft().fontRenderer.drawString(planet.getSpaceBodyName(), -width / 2, 0, Planet.getGuiColor(planet).getColor());

            if (planet.isHomeworld(Minecraft.getMinecraft().player)) {
                width = Minecraft.getMinecraft().fontRenderer.getStringWidth("[Home]");
                Minecraft.getMinecraft().fontRenderer.drawString(TextFormatting.GOLD + "[Home]", -width / 2, -10, 0xFFFFFF);
            }
        } else {
            ...
            if (planet.hasOwner()) {
                ...
                if (owner != null) {
                    ...
                    Minecraft.getMinecraft().fontRenderer.drawString(TextFormatting.GOLD + info, -width / 2, -10, 0xFFFFFF);
                }
            }
        }
    }
