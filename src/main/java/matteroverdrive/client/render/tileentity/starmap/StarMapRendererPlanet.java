                ...
                for (int i = 0; i < planet.getSizeInventory(); i++) {
                    if (planet.getStackInSlot(i) != null) {
                        ...
                        if (stack.getItem() instanceof IBuildable && planet.canBuild((IBuildable) stack.getItem(), stack, info)) {
                            ...
                            GlStateManager.enableBlend();
                            GlStateManager.blendFunc(GL_ONE, GL_ONE);
                            RenderUtils.drawString(String.format("%1$s - %2$s", stack.getDisplayName(), MOStringHelper.formatRemainingTime(((IBuildable) stack.getItem()).getRemainingBuildTimeTicks(stack, planet, Minecraft.getMinecraft().world) / 20)), 18, y + 5 - itemCount * 18 - 21, Reference.COLOR_HOLO, opacity);
                        } else {
                            RenderUtils.renderStack(0, y - itemCount * 18 - 21, 0, stack, false);
                            GlStateManager.enableBlend();
                            GlStateManager.blendFunc(GL_ONE, GL_ONE);
