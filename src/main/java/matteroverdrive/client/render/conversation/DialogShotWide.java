...
public class DialogShotWide extends DialogShot {
    ...
    public boolean positionCamera(EntityLivingBase active, EntityLivingBase other, float ticks, EntityRendererConversation rendererConversation) {
        ...
        Vec3d center = rendererConversation.getPosition(active, ticks).addVector(centerDir.x / 2, centerDir.y / 2, centerDir.z / 2);
        ...
        Vec3d pos = center.addVector(centerCross.x * distance, centerCross.y * distance, centerCross.z * distance);
        if (hit != null) {
            pos = hit.hitVec;
        }
