...
public class BioticStatTeleport extends AbstractBioticStat implements IConfigSubscriber {
    ...
    public Vec3d getPos(AndroidPlayer androidPlayer) {
        RayTraceResult position = MOPhysicsHelper.rayTraceForBlocks(androidPlayer.getPlayer(), androidPlayer.getPlayer().world, MAX_TELEPORT_DISTANCE, 0, new Vec3d(0, androidPlayer.getPlayer().getEyeHeight(), 0), true, true);
        ...
        position = MOPhysicsHelper.rayTrace(androidPlayer.getPlayer(), androidPlayer.getPlayer().world, 6, 0, new Vec3d(0, androidPlayer.getPlayer().getEyeHeight(), 0), true, true);
        if (position != null) {
    ...
    private BlockPos getTopSafeBlock(World world, BlockPos pos, EnumFacing side) {
        ...
        for (int i = pos.getY(); i < height; i++) {
            ...
            if (block.getBlock().isAir(block, world, blockPos) || !block.getBlock().isCollidable() || block instanceof IFluidBlock) {
                airBlockCount++;
            } else {
                airBlockCount = 0;
            }
