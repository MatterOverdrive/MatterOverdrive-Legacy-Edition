            ...
            if (getSlot().getStack() != null) {
                if (getSlot().getStack().getItem() instanceof IShip) {
                    ...
                    if (starMap.getPlanet().canBuild((IShip) getSlot().getStack().getItem(), getSlot().getStack(), info)) {
                        ...
                        String time = MOStringHelper.formatRemainingTime(((IShip) shipStack.getItem()).getRemainingBuildTimeTicks(shipStack, starMap.getPlanet(), Minecraft.getMinecraft().world) / 20);
