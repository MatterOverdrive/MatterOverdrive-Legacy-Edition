...
public class RenderUtils {
    ...
    public static void renderStack(int x, int y, int z, ItemStack stack, boolean renderOverlay) {
        if (!stack.isEmpty()) {
            ...
            if (renderOverlay) {
                renderItemOverlayIntoGUI(font, stack, x, y, stack.getCount() > 1 ? Integer.toString(stack.getCount()) : null);
            }
	...
	/*public static void tesseleteModelAsBlock(Matrix4f mat,GroupObject object,IIcon icon,int x,int y,int z,int brightness,boolean lighting,Color color)
	{
		...
		for (Face face : object.faces)
		{
			...
			{
				for (int i = 0; i < face.textureinates.length; ++i)
				{
					averageU += face.textureinates[i].u;
					averageV += face.textureinates[i].v;
				}

				averageU = averageU / face.textureinates.length;
				averageV = averageV / face.textureinates.length;
			}
			...
			for (int i = 0; i < face.vertices.length; ++i)
			{
				...
				if ((face.textureinates != null) && (face.textureinates.length > 0))
				{
					...
					if (face.textureinates[i].u > averageU)
					{
						offsetU = -offsetU;
					}
					if (face.textureinates[i].v > averageV)
					{
						offsetV = -offsetV;
					}
					...
					Tessellator.instance.addVertexWithUV(x + 0.5f + pos.x, y + pos.y, z + 0.5f + pos.z, icon.getMinU() + face.textureinates[i].u * uSize + offsetU, icon.getMinV() + face.textureinates[i].v * vSize + offsetV);
				}
				else
				{
    ...
    public static void rotateTowards(Vec3d from, Vec3d to, Vec3d up) {
        ...
        GlStateManager.rotate((float) (rotAngle * (180d / Math.PI)), (float) rotAxis.x, (float) rotAxis.y, (float) rotAxis.z);
    }
