...
public abstract class MOTileEntityMachine extends MOTileEntity implements IMOTileEntity, ISidedInventory, IUpgradeable, ITickable {
    ...
    public void SpawnVentParticles(float speed, EnumFacing side, int count) {
        for (int i = 0; i < count; i++) {
            ...
            VentParticle ventParticle = new VentParticle(this.world, this.getPos().getX() + offset.x + circleTransformed.x, this.getPos().getY() + offset.y + circleTransformed.y, this.getPos().getZ() + offset.z + circleTransformed.z, side.getDirectionVec().getX() * speed, side.getDirectionVec().getY() * speed, side.getDirectionVec().getZ() * speed, scale);
