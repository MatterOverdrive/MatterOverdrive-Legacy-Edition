...
public class PlasmaBolt extends Entity implements IProjectile, IGravityEntity, IEntityAdditionalSpawnData {
    ...
    public PlasmaBolt(World world, EntityLivingBase entityLivingBase, Vec3d position, Vec3d dir, WeaponShot shot, float speed) {
        ...
        this.setSize(0.5F, 0.5F);
        this.setLocationAndAngles(position.x, position.y, position.z, entityLivingBase.rotationYaw, entityLivingBase.rotationPitch);
        this.motionX = dir.x;
        this.motionY = dir.y;
        this.motionZ = dir.z;
        this.height = 0.0F;
    ...
    public void onUpdate() {
        ...
        if (movingobjectposition != null) {
            if (movingobjectposition.entityHit != null) {
                ...
                MinecraftForge.EVENT_BUS.post(new MOEventPlasmaBlotHit(weapon, movingobjectposition, this, world.isRemote ? Side.CLIENT : Side.SERVER));
            } else {
    ...
    protected void handleRicochets(RayTraceResult hit) {
        ...
        Vec3d projection = new Vec3d(surfaceNormal.x * deflectDot, surfaceNormal.y * deflectDot, surfaceNormal.z * deflectDot);
    ...
    protected void onHit(RayTraceResult hit) {
        ...
        if (hit.typeOfHit.equals(RayTraceResult.Type.ENTITY)) {
            if (hit.entityHit instanceof EntityLivingBase) {
                EntityFXGenericAnimatedParticle blood = new EntityFXGenericAnimatedParticle(world, hit.hitVec.x + rand.nextDouble() * 0.4 - 0.2, hit.hitVec.y + rand.nextDouble() * 0.4 - 0.2, hit.hitVec.z + rand.nextDouble() * 0.4 - 0.2, 6f + rand.nextFloat() * 2, RenderParticlesHandler.blood);
            ...
            for (int i = 0; i < hitPraticles; i++) {
                Minecraft.getMinecraft().effectRenderer.addEffect(new PhaserBoltRecoil(world, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, c, sideHit.x * 30, sideHit.y * 30, sideHit.z * 30));
            }
            ...
            if (rand.nextBoolean()) {
                EntityFXGenericAnimatedParticle smoke = new EntityFXGenericAnimatedParticle(world, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, 8f + rand.nextFloat() * 2, RenderParticlesHandler.smoke);
    ...
    private boolean manageExplosions(RayTraceResult hit) {
        if (explodeMultiply > 0) {
            if (!world.isRemote) {
                Explosion explosion = new Explosion(world, this, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, explodeMultiply, false, false);
                if (!net.minecraftforge.event.ForgeEventFactory.onExplosionStart(world, explosion)) {
                    explosion.doExplosionA();
                    this.world.playSound(null, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, MatterOverdriveSounds.weaponsExplosiveShot, SoundCategory.PLAYERS, 3.0F, (1.0F + (this.world.rand.nextFloat() - this.world.rand.nextFloat()) * 0.2F) * 0.7F);
                }
            } else {
    ...
    private void manageClientExplosions(RayTraceResult hit) {
        ...
        EntityFXGenericAnimatedParticle explosion = new EntityFXGenericAnimatedParticle(world, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, 10, RenderParticlesHandler.explosion);
        ...
        for (int i = 0; i < 6; i++) {
            explosion = new EntityFXGenericAnimatedParticle(world, hit.hitVec.x + rand.nextGaussian() * explosionRange, hit.hitVec.y + rand.nextGaussian() * explosionRange, hit.hitVec.z + rand.nextGaussian() * explosionRange, 4 + rand.nextFloat() * 4, RenderParticlesHandler.explosion);
            explosion.setRenderDistanceWeight(3f);
            explosion.setParticleMaxAge(18);
            ClientProxy.renderHandler.getRenderParticlesHandler().addEffect(explosion, RenderParticlesHandler.Blending.Transparent);
        }
