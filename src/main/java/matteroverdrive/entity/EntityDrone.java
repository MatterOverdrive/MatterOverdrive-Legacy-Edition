        ...
        if (world.isRemote) {
            if (ticksExisted % 2 == 0) {
                ...
                for (int i = 0; i < motionSpeed; i++) {
                    world.spawnParticle(EnumParticleTypes.CRIT_MAGIC, posX - look.x * 0.25, posY + getEyeHeight() + 0.2 - look.y * 0.2, posZ - look.z * 0.2, -motionX * 2, -motionY * 2, -motionZ * 2);
                }
            }
/*
			IBlockState bottomBlock = world.getBlockState(target.offset(EnumFacing.DOWN));
            if (bottomBlock.getBlock() != Blocks.air)
            {
                ...
                for (int i = 0;i < 3;i++)
                {
                    ...
                    world.spawnParticle(EnumParticleTypes.BLOCK_DUST, x, target.getY(), z, dirX, 0.15, dirZ, blockID);
                }
            }*/
        }
