                    ...
                    if (hit.typeOfHit == RayTraceResult.Type.BLOCK && player.world.getBlockState(pos).getBlock().isFlammable(player.world, pos, hit.sideHit)) {
                        ...
                        if (player.canPlayerEdit(pos, hit.sideHit, item)) {
                            if (player.world.isAirBlock(pos)) {
                                //player.world.playSoundEffect((double) x + 0.5D, (double) y + 0.5D, (double) z + 0.5D, "fire.ignite", 1.0F, itemRand.nextFloat() * 0.4F + 0.8F);
        ...
        if (hit.entityHit != null && hit.entityHit instanceof EntityLivingBase) {
            if (WeaponHelper.hasStat(Reference.WS_HEAL, weapon)) {
                ...
                if (isKillMode(weapon)) {
                    ...
                    world.spawnParticle(EnumParticleTypes.CRIT_MAGIC, hit.hitVec.x, hit.hitVec.y, hit.hitVec.z, 0, 0, 0);
                }
            }
